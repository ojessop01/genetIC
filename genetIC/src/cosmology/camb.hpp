...
namespace cosmology {
  ...
  protected:
    ...
    //! User-specified k_min for alpha calculation; negative means use full CAMB range
    CoordinateType alphaKMin_ = -1.0;
    //! User-specified k_max for alpha calculation; negative means use full CAMB range
    CoordinateType alphaKMax_ = -1.0;
  ...
  public:
     ...
     * \param filename Path to CAMB transfer function file
     * \param computeIsocurvature Whether to compute isocurvature alpha coefficient
     * \param computeVbvcVariance Whether to compute vb-vc variance
     * \param alphaKMin User-specified k_min for alpha calculation (h/Mpc); negative uses full CAMB range
     * \param alphaKMax User-specified k_max for alpha calculation (h/Mpc); negative uses full CAMB range
     */
    CAMB(const CosmologicalParameters<CoordinateType> &cosmology, const std::string &filename,
         ...
         CoordinateType alphaKMin = -1.0, CoordinateType alphaKMax = -1.0)
      : cosmology_(cosmology), alphaKMin_(alphaKMin), alphaKMax_(alphaKMax) {
     ...
     *
     * If alphaKMin_ and alphaKMax_ are set (positive values), the integration
     * is restricted to that k-range. Otherwise, the full CAMB table is used.
     */
    CoordinateType calculateAlphaCoefficientDiscrete() const {
      ...
      // Determine effective k-range for integration
      const bool useCustomKRange = (alphaKMin_ > 0 && alphaKMax_ > 0);
      const CoordinateType kmin_eff = useCustomKRange ? alphaKMin_ : static_cast<CoordinateType>(kcamb.front());
      const CoordinateType kmax_eff = useCustomKRange ? alphaKMax_ : static_cast<CoordinateType>(kcamb.back());

      if (useCustomKRange) {
        logging::entry() << "Alpha calculation using k-range: [" << kmin_eff << ", " << kmax_eff << "] h/Mpc" << std::endl;
      } else {
        logging::entry() << "Alpha calculation using full CAMB k-range: [" << kcamb.front() << ", " << kcamb.back() << "] h/Mpc" << std::endl;
      }
      ...
      for (size_t i = 0; i + 1 < n; ++i) {
        ...
        // Skip bins outside the effective k-range
        if (k2 < kmin_eff || k1 > kmax_eff) continue;

        // Clamp bin edges to the effective k-range
        const CoordinateType k1_clamped = std::max(k1, kmin_eff);
        const CoordinateType k2_clamped = std::min(k2, kmax_eff);

        // Skip if clamped range is invalid
        if (k1_clamped >= k2_clamped) continue;
    
        // Logarithmic bin width in k (using clamped values)
        const CoordinateType dlnk = std::log(k2_clamped / k1_clamped);
    
        // Baryon–CDM mode at each endpoint (using original table values)
        ...
        // Total matter mode (using original table values)
        const CoordinateType Tm1 = static_cast<CoordinateType>(Tm[i]);
        const CoordinateType Tm2 = static_cast<CoordinateType>(Tm[i + 1]);
    
        // Cross spectra (use clamped k values for the k^(ns+3) factor)
        const CoordinateType Pmbc1 = std::pow(k1_clamped, ns + 3) * Tbc1 * Tm1;
        const CoordinateType Pmbc2 = std::pow(k2_clamped, ns + 3) * Tbc2 * Tm2;
    
        const CoordinateType Pm1 = std::pow(k1_clamped, ns + 3) * Tm1 * Tm1;
        const CoordinateType Pm2 = std::pow(k2_clamped, ns + 3) * Tm2 * Tm2;
      ...
      const CoordinateType alpha_z0 = num / den;

      // ------------------------------------------------------------------
      // Physical sanity checks for ΛCDM alpha at z=0.
      // Expectation:
      //   alpha < 0  (anti-correlation of δ_bc with δ_m)
      //   |alpha| < 1  (bounded correlation strength)
      // ------------------------------------------------------------------
      if (!std::isfinite(alpha_z0)) {
      ...
      if (std::abs(alpha_z0) >= static_cast<CoordinateType>(1.0) + eps) {
        throw std::runtime_error(
          "Computed alpha violates ΛCDM expectation: |alpha| must be < 1.");
      }

      // ------------------------------------------------------------------
      // Rescale alpha from z=0 to the IC redshift.
      // 
      // The CAMB transfer functions are normalized to z=0, so the computed
      // alpha is α(z=0). The isocurvature mode δ_bc is a compensated mode
      // (constant in amplitude, doesn't grow), while matter overdensity δ_m
      // grows as D(z). Therefore:
      //
      //   α(z) = <δ_bc · δ_m(z)> / <δ_m(z)²>
      //        = <δ_bc · D(z)δ_m(0)> / <[D(z)δ_m(0)]²>
      //        = α(z=0) / D(z)
      //
      //   α(z_IC) = α(z=0) / [D(z_IC) / D(z=0)]
      // 
      // where D(z_IC/z=0) = D(z_IC) / D(z=0) is the growth factor ratio.
      // ------------------------------------------------------------------
      ...
      logging::entry() << "Growth factor D(z=" << cosmology_.redshift << ")/D(z=0) = " << growthFactorNormalized << std::endl;

      logging::entry() << "Rescaled alpha at z=" << cosmology_.redshift << " = " << alpha << std::endl;
